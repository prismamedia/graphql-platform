...
describe('Dependency', () => {
  ...
  describe('With virtual-selection', () => {
    ...
    it('has a consistent dependency-tree', (t) => {
      ...
      t.assert.snapshot(dependencyTree.createImpactTree(changes)?.toJSON());
    });
  });
  ...
  describe('Pure', () => {
    ...
    (
      ...
      it(`discards change(s): ${reason}`, (t) => {
        t.assert.equal(
          dependencyTree
            .createImpactTree(MutationContextChanges.createFromChanges(changes))
            ?.toJSON(),
          undefined,
        );
      }),
    ...
    before(() => {
      dependencyTree = new DocumentSetDependency(Article, {
        ...
        selection: Article.outputType.select(`{
          ...
          tags(where: { tag: { deprecated_not: true }}, first: 5) {
            tag {
              slug
            }
