...
import { INodeValue, TOrderByNodeValue, TWhereNodeValue } from '../../node';
import {
...
export interface IFindOperationArgs extends ISelectionsAwareOperationArgs {
  ...
  readonly orderBy?: TOrderByNodeValue;
...
> {
  ...
  public readonly description = `Retrieves a list of "${this.node}" nodes`;
  ...
  protected get graphqlFieldConfigArgs(): TOperationFieldArgs<IFindOperationArgs> {
    return {
      ...
      ...(this.node.orderByInput.type
        ? {
            orderBy: {
              type: this.node.orderByInput.type,
