...
export class GetOperation extends AbstractQuery<
  ...
  TGetOperationResult
> {
  public readonly name = camelize(this.node.name, true);
  public readonly description = `Retrieves one "${this.node}" node, throws an Error if it does not exist`;
  ...
  protected async doExecute(
    ...
    const [nodeValue = null] = await this.node.getOperation('find').execute(
      {
        where: this.node.whereUniqueInput.assertValue(
          where,
          addPath(path, 'where'),
        ),
        first: 1,
