...
import { Edge, Leaf, Scalars } from '../../components';
import { ReverseEdge } from '../../reverse-edge';
import { TFilterValue, TLeafFilterOperator } from './where/ast';
...
export type TWhereNodeValue = { [filterName: string]: any };
...
export class WhereNodeInput {
  ...
  protected getLeafFilters(leaf: Leaf): Filter[] {
    ...
    if (
      ...
      for (const operator of <TLeafFilterOperator[]>[
        'gt',
        'gte',
        'lt',
        'lte',
  ...
  public get type(): GraphQLInputObjectType {
    ...
    const publicFilterMap = new Map([...this.filterMap].filter(isPublicEntry));
    assert(
      publicFilterMap.size > 0,
    ...
    return new GraphQLInputObjectType({
      ...
      fields: () =>
        Object.fromEntries(
          Array.from(publicFilterMap.values(), (filter) => [
            filter.name,
  ...
  public parseValue(
    value: TWhereNodeValue | boolean | null | undefined,
    path?: Path,
  ): TFilterValue {
    if (value === null) {
      return {
