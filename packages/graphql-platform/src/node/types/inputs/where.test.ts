...
import { TWhereNodeValue, WhereNodeInput } from './where';
import { TEdgeFilterValue, TFilterValue } from './where/ast';
...
describe('WhereInput', () => {
  ...
  it.each(nodeNames)('may have a "%sWhereInput" type', (nodeName) => {
    const node = gp.getNode(nodeName);
    const whereInput = node.whereInput;

    expect(whereInput).toBeInstanceOf(WhereNodeInput);
  ...
  it.each<[string, TWhereNodeValue | boolean | null | undefined, TFilterValue]>(
    [
