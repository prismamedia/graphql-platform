...
export type TMaybeNodeAware<TConnector extends IConnector, T> =
  | T
  | ((node: Node<TConnector>) => T);

export const getMaybeNodeAware = <TConnector extends IConnector, T>(
  node: Node<TConnector>,
  config?: TMaybeNodeAware<TConnector, T>,
): T | undefined =>
  config
...
export class CustomField extends AbstractField {
  ...
  @Memoize()
  public get dependsOn(): ReadonlyArray<TNodeOutputSelection> | undefined {
