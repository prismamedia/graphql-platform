...
export function getSelectionKey(selection: TNodeOutputSelection): string {
  return ('alias' in selection && selection.alias) || selection.name;
}

export function getSelectedEdge(
  node: Node,
  selection: TNodeOutputSelection<'Edge' | 'EdgeExistence'>,
): Edge {
  return node.getEdge(
...
export function getSelectedReverseEdge(
  node: Node,
  selection: TNodeOutputSelection<
    | 'ReverseEdge'
    | 'ReverseEdgeCount'
    | 'UniqueReverseEdge'
...
export function getNormalizedSelectionArgs(
  ...
  return 'args' in selection && selection.args
    ? normalizeObject(selection.args as any)
    : undefined;
}

export function mergeSelections(
  ...selections: TNodeOutputSelection[]
): TNodeOutputSelection[] {
  let mergedSelections: TNodeOutputSelection[] = [];
...
export function parseASTSelections(
  ...
  switch (ast.kind) {
    ...
    case 'SelectionSet': {
      let selections: TNodeOutputSelection[] = [];
...
export type TSelections =
  ...
  | GraphQLResolveInfo
  | TFragment
  | ReadonlyArray<TNodeOutputSelection>;

export function isSelection(
  maybeSelection: unknown,
): maybeSelection is TNodeOutputSelection {
  return (
...
): maybeSelections is ReadonlyArray<TNodeOutputSelection> {
  return (
