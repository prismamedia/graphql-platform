...
import { IEdgeSelection } from '../output/fields';
...
> = IComponentConfig<'Edge'> & {
  /**
   * Optional, either this edge is exposed publicly (in the GraphQL API) or not (only available internally)
   *
   * Default: public if its node and its referenced node are public
   */
  public?: OptionalFlag;
...
> extends AbstractComponent {
  public constructor(
    ...
    assert(
      ...
      `The "${this}" edge expects a non-empty string, got "${config.type}"`,
    );
  }
  ...
  public get reference(): UniqueConstraint<TConnector> {
    ...
    assert(
      ...
      `The "${this}" edge cannot references itself`,
    );
  ...
  public get public(): boolean {
    ...
    assert(
      ...
      `The "${this}" edge cannot be public as the node "${this.node}" is not`,
    );

    assert(
      !isPublic || this.to.public,
      `The "${this}" edge cannot be public as the referenced node "${this.to}" is not`,
    );
