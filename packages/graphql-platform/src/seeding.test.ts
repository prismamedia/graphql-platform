...
describe('Seeding', (t) => {
  ...
  it('checks for circular dependencies', () => {
    assert.throws(
      ...
      { message: 'Dependency Cycle Found: root -> home -> root' },
    );
  });
  ...
  it('orders by dependencies', () => {
    ...
    assert.strictEqual(seeding.fixtures.length, 15);
    assert.deepStrictEqual(
      seeding.fixtures.map(({ reference }) => reference),
      [
        'category_root',
        'category_home',
        'article_01',
  ...
  it('loads the fixtures', async (t) => {
            ...
            switch (node.name) {
              ...
              case 'UserProfile':
                return filter?.filter instanceof EdgeExistsFilter &&
                  ...
                  filter.filter.headFilter?.filter instanceof
                    LeafComparisonFilter
    ...
    const seeding = new Seeding(gp, fixtures.constant);
    await seeding.load(myAdminContext);
