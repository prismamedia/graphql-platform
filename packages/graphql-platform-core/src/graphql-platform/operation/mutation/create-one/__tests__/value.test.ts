...
describe('Resource', () => {
  ...
  it('creates a value', () => {
    ...
    expect(create.toNodeValue()).toEqual({
      title: 'My new title',
      slug: 'my-new-title',
    });
    ...
    expect(() => delete proxy.title).toThrowError(
      'The "Tag.title"\'s value is invalid: cannot be undefined.',
    );

    // Cannot set an invalid value
    expect(() => (proxy.title = undefined)).toThrowError(
      'The "Tag.title"\'s value is invalid: cannot be undefined.',
    );
