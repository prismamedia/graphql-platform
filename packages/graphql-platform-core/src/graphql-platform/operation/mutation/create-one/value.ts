import { fromEntries, MaybeUndefinedDecorator } from '@prismamedia/graphql-platform-utils';
import {
  Component,
  ComponentMap,
  ComponentValue,
...
const createInputHookProxyHandler: ProxyHandler<CreateOneValue> = {
  ...
  get(create, propertyKey) {
    ...
    const component = getComponentByPropertyKey(create.resource.getComponentMap(), propertyKey);
    if (!component) {
...
export type CreateOneRawValue = Partial<{ [componentName: string]: ComponentValue }>;
...
export class CreateOneValue {
  ...
  public set<TComponent extends AnyComponent>(
    ...
    value: (TComponent extends AnyField ? FieldValue : RelationValue) | undefined,
  ): boolean {
    ...
    if (typeof value === 'undefined') {
      ...
      this.data.set(component, component.isField() ? component.assertValue(value) : component.assertValue(value, true));
    }
  ...
  public get<TComponent extends AnyComponent, TStrict extends boolean>(
    component: TComponent,
    strict?: TStrict,
  ): MaybeUndefinedDecorator<TComponent extends AnyField ? FieldValue : RelationValue, TStrict> {
  ...
  public toNodeValue(): Partial<NodeValue> {
    return fromEntries([...this.data].map(([component, componentValue]) => [component.name, componentValue]));
  }
