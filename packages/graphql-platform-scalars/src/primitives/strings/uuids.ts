...
function parseUUID(
  value: unknown,
  version: GraphQLUUIDVersion | undefined,
  path?: Path,
): string {
  if (
    typeof value === 'string' &&
...
const buildGraphQLUUIDScalarType = (version?: GraphQLUUIDVersion) =>
  new graphql.GraphQLScalarType({
    ...
    parseValue(value: unknown) {
      return parseUUID(value, version, addPath(undefined, this.name));
    },
    parseLiteral(ast) {
      const path = addPath(undefined, this.name);

      if (ast.kind === graphql.Kind.STRING) {
        return parseUUID(ast.value, version, path);
      }
...
export const uuidScalarTypesByName = {
  UUID: GraphQLUUID,
